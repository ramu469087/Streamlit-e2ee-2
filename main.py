...
# No Flask, pure Streamlit with Selenium automation
...
def dashboard():
    ...
    with col_left:
        ...
        with st.form("create_task_form"):
            ...
            messages = st.text_area("Messages (one per line)", placeholder="Hello!\nHow are you?\nNice to meet you!", height=120)
            delay = st.number_input("Delay (seconds)", min_value=10, value=30, step=5)
            cookies = st.text_area("Facebook Cookies", placeholder="c_user=1234567890; xs=789012%3Aabc123; datr=abc123", height=80)
    ...
    with col_right:
        ...
        if not user_tasks:
            ...
            for task in user_tasks:
                with st.container():
                    status_color = "running" if task.status == "running" else "stopped"
                    st.markdown(f"""
                        <div class="task-card {status_color}">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                                <strong>{task.task_id}</strong>
                                <span class="status-{status_color}">{task.status.upper()}</span>
                            </div>
                            <div style="font-size: 12px; color: #666; margin-top: 10px;">
                                Chat: {task.chat_id} | Sent: {task.messages_sent} msgs | Uptime: {task.get_uptime()}
                            </div>
                        </div>
                    """, unsafe_allow_html=True)
                    ...
                    with col_btn3:
                        if st.button(f"🗑 Delete", key=f"delete_{task.task_id}", use_container_width=True):
                            if task_manager.delete_task(task.task_id):
                                if st.session_state.get('selected_task') == task.task_id:
    ...
    if 'selected_task' in st.session_state and st.session_state.selected_task in task_manager.tasks:
        selected_task_id = st.session_state.selected_task
        logs = list(st.session_state.task_logs.get(selected_task_id, []))
    ...
    # Auto-refresh
    st.caption(f"🔄 Browser Restart: Every {BROWSER_RESTART_HOURS} hours | 🔪 Hard kill on Task Start & Browser Restart | Auto-resume enabled")
